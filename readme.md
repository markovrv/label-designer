...
Этот репозиторий содержит веб-приложение для создания и редактирования дизайна этикеток с возможностью печати на термопринтере Bixolon. Приложение представляет собой визуальный редактор, позволяющий создавать макеты этикеток с текстовыми полями, изображениями и другими элементами, а затем сохранять их и использовать для печати на принтере Bixolon XD3-40d через Web Print SDK.
...
- **Печать на термопринтере Bixolon**: Прямая интеграция с принтером Bixolon XD3-40d через Web Print SDK
- **Шаблонизация**: Использование переменных в макетах (например, `{{variableName}}`) для подстановки данных при печати
...
- **Поддержка плейсхолдеров**: Автоматическая замена переменных в макетах при печати

### Структура проекта

- `server.js` - основной серверный код приложения (Express)
- `bixolon/BixolonPrinter.js` - класс для взаимодействия с принтером Bixolon
...
- `BIXOLON_HOST` - адрес хоста сервера печати Bixolon (по умолчанию localhost)
- `BIXOLON_PORT` - порт сервера печати Bixolon (по умолчанию 18080)
- `BIXOLON_PRINTER_NAME` - имя принтера в Web Print SDK (по умолчанию LabelPrinter)
- `BIXOLON_TIMEOUT` - таймаут для HTTP запросов к Web Print SDK (по умолчанию 30000)
- `BIXOLON_DEBUG` - режим отладки (по умолчанию false)

### Установка и запуск

#### Локальный запуск:
1. Установите Node.js
2. Установите и запустите Web Print SDK от Bixolon на вашем компьютере
...
docker run -p 3000:3000 -e BIXOLON_HOST=host.docker.internal label-designer
```

Для доступа к принтеру с контейнера на хост-машине используйте `host.docker.internal` в качестве адреса хоста принтера.
